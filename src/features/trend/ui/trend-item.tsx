...
export const TrendItem = ({ data, ...props }: TrendItemProps) => {
    ...
    return (
        <div {...props} draggable={false} className={twMerge("space-y-3", props.className)}>
            ...
            <div className="flex text-[10px] text-gray-500 font-medium dark:text-gray-400">
                <p className="max-w-1/2 truncate">{data?.author || "Unknown"}</p><p>&nbsp;•&nbsp;{DateTime.fromISO(data.createdAt).toFormat("yyyy. MM. dd")}</p>
            </div> 
            <h2 className="text-xl font-semibold truncate max-sm:text-lg">{data.title}</h2>
        </div>
    );
}